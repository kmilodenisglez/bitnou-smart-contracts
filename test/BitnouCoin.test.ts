...
describe('BitnouCoin (Testnet Config)', () => {
  ...
  describe('Metadata', () => {
    it('has the correct testnet name', async () => {
      const { bitnou } = await getFixture()
      expect(await bitnou.read.name()).to.equal('BitnouCoin Testnet')
    })

    it('has the correct testnet symbol', async () => {
      const { bitnou } = await getFixture()
      expect(await bitnou.read.symbol()).to.equal('tBNOU')
    })
